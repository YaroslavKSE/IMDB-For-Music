...
public class GetInteractionsByItemIdsUseCase : IRequestHandler<GetInteractionsByItemIdsCommand, GetInteractionsResult>
{
    private readonly IInteractionStorage interactionStorage;

    public GetInteractionsByItemIdsUseCase(IInteractionStorage interactionStorage)
    {
        this.interactionStorage = interactionStorage;
    }

    public async Task<GetInteractionsResult> Handle(GetInteractionsByItemIdsCommand request, CancellationToken cancellationToken)
    {
        ...
        var paginatedResult = await interactionStorage.GetInteractionsByItemIds(request.ItemIds, request.Limit, request.Offset);
